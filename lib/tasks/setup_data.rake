...
#Populating Tables
	...
	5.times do
		Venue.create(user_id: venue_owners.sample, name: "#{Faker::Lorem.word.capitalize}", locality_id: Locality.ids.sample, address: Faker::Address.street_address, seats: [250,500,750,1000].sample)
	end
	...
	fests = ["Fest", "Show", "Unplugged", "2016", "Workshop", "Concert", "Festival 2016"]
	organizers = [user1.id, user2.id]
	10.times do 
		startdate = Faker::Time.between(DateTime.now, DateTime.now + 7)
		event = Event.create(user_id: organizers.sample, category_ids: Category.ids.sample(2), name: "#{Faker::Lorem.word.capitalize}" + " #{fests.sample}", description: Faker::Lorem.paragraph, venue_id: Venue.ids.sample, start_datetime: startdate, end_datetime: Faker::Time.between(startdate + 1.day, startdate + 3.days), entry_fee: Faker::Number.between(500,1500),tickets_left: [250,500,750,1000].sample, status: statuses.sample, locality_id: Locality.ids.sample )
